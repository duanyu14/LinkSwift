...
(function linkSwift($) {
	...
	let info = {
		...
		version: (GM_info.script?.version || "1.2.1.2"),
	...
	let base = {
		...
		initDefaultConfig() {
			...
			let defaults = [
				...
				{
					name: "setting_default_mode",
					value: "api"
				},
				{
					name: "setting_enabled_pan",
					value: {
						$baidu: true,
						$aliyun: true,
						$mcloud: true,
		 ...
		 * 获取当前默认下载方式的中文名称
		 * @author 烟雨烟中客
		 * @returns {String} 下载方式名称，如 "API 下载"
		 */
		getDefaultModeName() {
			let mode = base.getValue("setting_default_mode") || "api";
			return config.base.dom.button[mode]?.title || "默认下载";
		},

		/**
		 * 刷新页面上所有默认下载按钮的文字
		 * @author 烟雨烟中客
		 * @description 将所有 .pl-default-mode-text 元素的文字更新为当前默认下载方式名称
		 */
		refreshDefaultModeButtons() {
			let name = base.getDefaultModeName();
			$(".pl-default-mode-text").text(name);
		},
		...
		showSetting(event) {
			let setting = $(`<div>
				...
				<label class="pl-setting-item listener-tip" data-title="点击「下载助手」旁的「默认下载」按钮时使用的下载方式">
					<div>默认下载方式</div>
					<div class="pl-ios-switches">
						<label class="pl-ios-switch-item"><input type="radio" name="default_mode" class="listener-default-mode" value="api" ${base.getValue("setting_default_mode") === "api" ? "checked" : ""}><span class="pl-ios-switch-label">API 下载</span></label>
						<label class="pl-ios-switch-item"><input type="radio" name="default_mode" class="listener-default-mode" value="curl" ${base.getValue("setting_default_mode") === "curl" ? "checked" : ""}><span class="pl-ios-switch-label">cURL 下载</span></label>
						<label class="pl-ios-switch-item"><input type="radio" name="default_mode" class="listener-default-mode" value="aria2" ${base.getValue("setting_default_mode") === "aria2" ? "checked" : ""}><span class="pl-ios-switch-label">Aria2 下载</span></label>
						<label class="pl-ios-switch-item"><input type="radio" name="default_mode" class="listener-default-mode" value="bitcomet" ${base.getValue("setting_default_mode") === "bitcomet" ? "checked" : ""}><span class="pl-ios-switch-label">彗星下载</span></label>
						<label class="pl-ios-switch-item"><input type="radio" name="default_mode" class="listener-default-mode" value="abdm" ${base.getValue("setting_default_mode") === "abdm" ? "checked" : ""}><span class="pl-ios-switch-label">ABDM 下载</span></label>
					</div>
				</label>
				<label class="pl-setting-item listener-tip" data-title="控制各网盘是否启用脚本功能，关闭后该网盘不会显示按钮">
					<div>网盘开关 <button type="button" class="pl-button-mini swal2-confirm swal2-styled listener-pan-toggle-all" style="margin-left:5px;padding:2px 8px;font-size:11px;">全选</button></div>
					<div class="pl-ios-switches">
						<label class="pl-ios-switch-item"><input type="checkbox" class="listener-pan-toggle" data-pan="$baidu" ${base.getValue("setting_enabled_pan").$baidu !== false ? "checked" : ""}><span class="pl-ios-switch-label">百度网盘</span></label>
						<label class="pl-ios-switch-item"><input type="checkbox" class="listener-pan-toggle" data-pan="$aliyun" ${base.getValue("setting_enabled_pan").$aliyun !== false ? "checked" : ""}><span class="pl-ios-switch-label">阿里云盘</span></label>
						<label class="pl-ios-switch-item"><input type="checkbox" class="listener-pan-toggle" data-pan="$mcloud" ${base.getValue("setting_enabled_pan").$mcloud !== false ? "checked" : ""}><span class="pl-ios-switch-label">移动云盘</span></label>
						<label class="pl-ios-switch-item"><input type="checkbox" class="listener-pan-toggle" data-pan="$tcloud" ${base.getValue("setting_enabled_pan").$tcloud !== false ? "checked" : ""}><span class="pl-ios-switch-label">天翼云盘</span></label>
						<label class="pl-ios-switch-item"><input type="checkbox" class="listener-pan-toggle" data-pan="$xunlei" ${base.getValue("setting_enabled_pan").$xunlei !== false ? "checked" : ""}><span class="pl-ios-switch-label">迅雷云盘</span></label>
						<label class="pl-ios-switch-item"><input type="checkbox" class="listener-pan-toggle" data-pan="$quark" ${base.getValue("setting_enabled_pan").$quark !== false ? "checked" : ""}><span class="pl-ios-switch-label">夸克网盘</span></label>
						<label class="pl-ios-switch-item"><input type="checkbox" class="listener-pan-toggle" data-pan="$uc" ${base.getValue("setting_enabled_pan").$uc !== false ? "checked" : ""}><span class="pl-ios-switch-label">UC 网盘</span></label>
						<label class="pl-ios-switch-item"><input type="checkbox" class="listener-pan-toggle" data-pan="$123pan" ${base.getValue("setting_enabled_pan").$123pan !== false ? "checked" : ""}><span class="pl-ios-switch-label">123 云盘</span></label>
					</div>
				</label>
		...
		async showUpdate() {
			const versions = [
{ version: "V1.2.1.2", content: `<div>1、修复 - 更新日志下拉框显示问题；</div><div>2、优化 - 更新日志界面结构。</div>` },
{ version: "V1.2.1.1", content: `<div>1、修复 - 更新日志下拉框显示问题；</div><div>2、优化 - 默认显示最新版本日志。</div>` },
...
{ version: "V1.0.0", content: `<div>1、增加"注入"功能（bushi）；</div><div>2、去除广告。</div>` }
			...
			const generateOptions = () => {
					return versions.map(v => {
						...
						return `<option value="${v.version}"${selected}>${v.version}</option>`;
					}).join('');
				};

			const versionData = [{"version": "V1.2.1.2", "content": "<div>1、修复 - 更新日志下拉框显示问题；</div><div>2、优化 - 更新日志界面结构。</div>"}, {"version": "V1.2.1.1", "content": "<div>1、修复 - 更新日志下拉框显示问题；</div><div>2、优化 - 默认显示最新版本日志。</div>"}, {"version": "V1.2.1", "content": "<div>1、优化 - 更新日志界面改为下拉框选择形式，默认显示最新版本；</div><div>2、修复 - 修复已知问题；</div><div>3、新增 - 新增若干功能。</div>"}, {"version": "V1.2.0", "content": "<div>1、新增 - 「默认下载」按钮，一键使用预设下载方式获取链接；</div><div>2、新增 - 「默认下载方式」设置，可在助手设置中选择；</div><div>3、新增 - 「网盘开关」功能，可单独控制各网盘是否启用脚本；</div><div>4、优化 - 助手设置界面新增 iOS 风格滑动开关；</div><div>5、更新 - 版本号至 V1.2.0。</div>"}, {"version": "V1.1.3", "content": "<div>LinkSwift 开发者在此祝您新春快乐！</div><div>爆竹声中一岁除，春风送暖入屠苏。LinkSwift 迎来功能更新：</div><div>1、新增 - IDM 客户端设置；</div><div>2、优化 - 链接缓存、浮动提示框；</div><div>3、适配 - 百度网盘分享页。</div>"}, {"version": "V1.1.2.1", "content": "<div>1、新增 - API 下载的推送到 IDM 功能；</div><div style=\"margin-left:10px\">（感谢 <a href=\"https://github.com/Night-stars-1\" target=\"_blank\">Night Stars</a> 的帮助）</div><div>2、修复 - 复制 Aria2、cURL 命令行错误。</div>"}, {"version": "V1.1.2", "content": "<div>1、适配 - 123 云盘新策略；</div><div>2、适配 - 夸克、UC 网盘分享页；</div><div>3、新增 - 增强下载的多块多线程支持；</div><div>4、优化 - 页面绿化的部分匹配规则；</div><div>5、优化 - 增强下载进度条样式。</div>"}, {"version": "V1.1.1.9", "content": "<div>1、修复 - 123 云盘下载视频变为缩略图。</div>"}, {"version": "V1.1.1.8", "content": "<div>1、修复 - 适配新版 123 云盘分享页。</div>"}, {"version": "V1.1.1.7", "content": "<div>1、修复 - 缺失声明 @connect 导致的问题。</div>"}, {"version": "V1.1.1.6", "content": "<div>1、<span style=\"color:#fff;background:${temp.color}\">废弃 - 百度网盘 BDUSS Cookie 相关代码，转向使用更安全的 AccessToken</span>；</div><div>2、废弃 - 百度网盘分享页面下载相关代码；</div><div>3、优化 - 下载窗口可在设置改变后动态修改界面。</div>"}, {"version": "V1.1.1.5", "content": "<div>1、新增 - AB Download Manager 下载方式；</div><div>2、优化 - 支持从设置页面一键返回下载窗口，无需重复获取链接。</div>"}, {"version": "V1.1.1.4", "content": "<div>1、适配 123 云盘新版页面。</div>"}, {"version": "V1.1.1.3", "content": "<div>1、修复夸克网盘无法获取下载链接的 Bug；</div><div>2、修复 API 下载无法复制全部链接。</div>"}, {"version": "V1.1.1.2", "content": "<div>1、修复无法删除第一项远程配置的 Bug。</div>"}, {"version": "V1.1.1.1", "content": "<div>1、修复推送至 Aria2 时推送成功但报错的 Bug。</div>"}, {"version": "V1.1.1", "content": "<div>1、配置文件格式更新，<span style=\"color:#fff;background:${temp.color}\">支持添加、删除、切换多个服务配置</span>；</div><div>2、支持比特彗星推送下载，<span style=\"color:#fff;background:${temp.color}\">原 RPC 已并入 Aria2 下载</span>；</div><div>3、界面<span style=\"color:#fff;background:${temp.color}\">增加 Font Awesome 图标！</span>更好看啦；</div><div>4、优化脚本代码、界面，运行更轻快；</div><div>5、修复上个版本遗存的问题。</div>"}, {"version": "V1.1.0.1", "content": "<div>1、修复查看 RPC 下载任务的 Bug。</div>"}, {"version": "V1.1.0", "content": "<div>1、支持 UC 网盘、123 云盘；</div><div>2、改进了网盘主题的注入方式；</div><div>3、聚合并重构了部分重复函数，对整体脚本逻辑进行了梳理和精简；</div><div>4、将脚本执行阶段从 document-body 适配为 document-start。</div>"}, {"version": "V1.0.9.7", "content": "<div>1、修复移动云盘下载错误；</div><div>2、优化代码，更好的错误识别；</div><div>3、去除了油小猴云服务。</div>"}, {"version": "V1.0.9.6", "content": "<div>1、支持在百度网盘中选择文件夹下载；</div><div>2、优化部分提示。</div>"}, {"version": "V1.0.9.5", "content": "<div>1、修复因代码逻辑错误而无法获取链接的 Bug。</div>"}, {"version": "V1.0.9.4", "content": "<div>1、修复因百度网盘 AccessToken 过期导致无法获取链接的 Bug。</div>"}, {"version": "V1.0.9.3", "content": "<div>1、若网盘不支持在分享中下载，将仅显示保存网盘按钮；</div><div>2、优化下载界面，支持选择 Iframe 或 Blob 的方式来下载文件，增加按钮的提示文本；</div><div>3、优化 CSS 样式，统一了 SweetAlert2 按钮样式，同时适配了 Dark Reader 插件，界面更协调；</div><div>4、支持修改油小猴网站主题色；</div><div>5、原有主题相关设置现已移动至助手美化页面中。</div>"}, {"version": "V1.0.9.2", "content": "<div>1、修复使用API 下载时有可能会导致IDM无限弹窗的Bug。</div>"}, {"version": "V1.0.9.1", "content": "<div>1、修复在百度网盘旧版下脚本无法删除元素的Bug。</div>"}, {"version": "V1.0.9", "content": "<div>1、跟进官方V6.2.7，修复因无法进行百度授权而导致获取直链报错 9019 的 Bug。</div>"}, {"version": "V1.0.8.9", "content": "<div>1、跟进官方V6.2.3，优化保存到网盘提示，修复阿里云盘、移动云盘失效的问题；</div><div>2、优化修改网盘主题的代码，减少对页面的破坏。</div>"}, {"version": "V1.0.8.8", "content": "<div>1、修复下载菜单字体过小的Bug。</div>"}, {"version": "V1.0.8.7", "content": "<div>1、修复在阿里云盘分享页面下点击\"未点亮\"按钮时没有任何反应的Bug；</div><div>2、更新并优化网盘界面精简规则；</div><div>3、支持更换 百度网盘、阿里云盘、迅雷云盘、夸克网盘、移动云盘 界面的主题颜色。</div>"}, {"version": "V1.0.8.6", "content": "<div>1、新增移动云盘会员中心页面，可在网盘中点击\"会员中心\"按钮查看(但无法使用第三方支付)。</div>"}, {"version": "V1.0.8.5", "content": "<div>1、跟进官方V6.1.6，修复迅雷网盘分享页面无法选中文件，修复移动云盘无法判断页面。</div>"}, {"version": "V1.0.8.4", "content": "<div>1、修复因重复绑定按钮而导致命令重复执行的Bug；</div><div>2、优化调试信息界面排版；</div><div>3、移除对百度网盘手机网页版的支持。</div>"}, {"version": "V1.0.8.3", "content": "<div>1、适配阿里云盘新域名alipan.com。</div>"}, {"version": "V1.0.8.2", "content": "<div>1、更换新图标。</div>"}, {"version": "V1.0.8.1", "content": "<div>1、修复因重复绑定按钮而导致 RPC 下载会发送多条下载请求的Bug；</div><div>2、选择不使用油小猴服务器时，\"用ghproxy连接Github仓库\"更换为\"用jsdelivr连接Github仓库\"；</div><div>3、跟进官方V6.1.4版本，修复移动网盘无法获取链接，支持阿里云盘新域名alipan.com。</div>"}, {"version": "V1.0.8", "content": "<div>1、修复迅雷网盘无法勾选文件。</div>"}, {"version": "V1.0.7.9", "content": "<div>1、更新精简网盘元素匹配规则，防止因通知横条而导致不能点到\"API 下载\"以下的按钮。</div>"}, {"version": "V1.0.7.8", "content": "<div>1、跟进官方V6.1.2，加入V2接口。</div><div>2、修复百度网盘下载时因为获取不到accessToken而一直转圈。</div>"}, {"version": "V1.0.7.7", "content": "<div>1、修复百度网盘的按钮会因为主题不同而被改变颜色的Bug；</div><div>2、更新夸克网盘按钮与界面。</div>"}, {"version": "V1.0.7.6", "content": "<div>1、修复\"注入\"功能；</div><div>2、黑暗模式支持随设置热切换。</div>"}, {"version": "V1.0.7.5", "content": "<div>1、修复阿里云盘下载逻辑；</div><div>2、精简代码；</div><div>3、支持深色模式；</div><div>4、修改部分提示文本；</div><div>5、修改部分CSS；</div><div>6、设置可测试RPC连接。</div>"}, {"version": "V1.0.7.4", "content": "<div>1、优化下载逻辑；</div><div>2、修复阿里云盘无法使用API 下载。</div>"}, {"version": "V1.0.7.3", "content": "<div>1、如果出现网络请求错误时支持自动重新请求；</div><div>2、可选择是否使用油小猴服务器。</div>"}, {"version": "V1.0.7.2", "content": "<div>1、修复使用 RPC 下载时会重复发送链接的Bug。</div>"}, {"version": "V1.0.7.1", "content": "<div>[实验功能，不影响正常使用]支持百度网盘手机网页版，勾选文件后可在顶栏找到\"下载助手\"按钮。</div>"}, {"version": "V1.0.7", "content": "<div>1、重构夸克网盘、阿里云盘按钮。</div>"}, {"version": "V1.0.6.9", "content": "<div>1、下载窗口加入关闭按钮。</div>"}, {"version": "V1.0.6.8", "content": "<div>1、修复夸克网盘按钮错位。</div>"}, {"version": "V1.0.6.7", "content": "<div>1、将百度网盘界面修改为主题色，可在设置选择是否修改；</div><div>2、增加主题色名称，更改部分内容颜色；</div><div>3、移动云盘API 下载支持批量复制；</div><div>4、优化控制台输出结果；</div><div>5、百度网盘API 下载不使用IDM时可以显示剩余时间；</div><div>6、\"取消点亮按钮\"按钮的位置现已移动到设置页面。</div><div>7、homo特有的彩蛋又回来力(喜)。</div>"}, {"version": "V1.0.6.6", "content": "<div>1、修复暗号错误。</div>"}, {"version": "V1.0.6.5", "content": "<div>1、修复即使输入正确暗号也不能成功点亮按钮的服务器错误。</div>"}, {"version": "V1.0.6.4", "content": "<div>1、跟进官方V6.1.1版本，修复阿里云盘获取下载链接时的问题。</div>"}, {"version": "V1.0.6.3", "content": "<div>1、照顾小屏幕用户，将始终显示复制全部链接的按钮；</div><div>2、增加取消下载时的动画。</div>"}, {"version": "V1.0.6.2", "content": "<div>1、修复部分界面错位，实现CSS内置；</div><div>2、百度网盘界面将变得更加简洁。</div>"}, {"version": "V1.0.6.1", "content": "<div>1、新增百度云盘API 下载支持复制链接；</div><div>2、为了照顾手机浏览器用户，增大项目之间间隙，新增隐藏IDM提示选项，可在助手设置中启用；</div><div>3、修改CSS，界面会出现更多的主题色；</div><div>4、支持在油小猴官网查看暗号；</div><div>5、修复部分语法错误。</div>"}, {"version": "V1.0.6", "content": "<div>1、修复了打开阿里云盘分享连接时因下载移动端广告导致只能点击 API 下载；</div><div>2、跟进官方6.0.4版本，修复夸克网盘获取下载链接失效、支持移动云盘。</div>"}, {"version": "V1.0.5.5", "content": "<div>1、感谢<a href=\"https://github.com/Night-stars-1\" target=\"_blank\">Night Stars</a>的帮助，修复因为原作者服务器导致的初始化暗号识别错误；</div><div>2、修改一些文本以及提供给服务器的信息。</div>"}, {"version": "V1.0.5.4", "content": "<div>1、小修小改css，让主题色出现在更多地方；</div><div>2、修改下载链接获取失败的提示；</div><div>3、增加更多的主题色，可在助手设置查看；</div><div>4、homo彩蛋被删去力（悲）。</div>"}, {"version": "V1.0.5.3", "content": "<div>1、修啦修啦，阿里云盘可以摸到下载菜单了。</div>"}, {"version": "V1.0.5.2", "content": "<div>1、增加脚本信息菜单（没有用）；</div><div>2、优化阿里云盘显示svg图片；</div><div>3、修改弹窗按钮颜色。</div>"}, {"version": "V1.0.5.1", "content": "<div>1、修复在切换按钮主题后夸克网盘不能正常显示按钮。</div>"}, {"version": "V1.0.5", "content": "<div>1、跟进官方V5.0.4版本；</div><div>2、小改动，照着官方版本更正文件名称检测；</div><div>3、保留彩蛋，但必须舍弃官方暗号。</div>"}, {"version": "V1.0.4", "content": "<div>大改！</div><div>1、修复了原作者留下的夸克网盘切换文件夹就多一个\"下载助手\"按钮的大BUG；</div><div>2、终于来了，在下载菜单增加\"助手设置\"\"更新日志\"按钮；</div><div>【再也不用点进油猴管理再进设置了(保留油猴管理内设置)】</div><div>3、修改阿里云盘和夸克网盘下载助手按钮样式；</div><div>4、增加\"取消点亮按钮\"油猴菜单；</div><div>5、修改部分css，使其与选择的主题更贴切。</div>"}, {"version": "V1.0.3", "content": "<div>1、增加一个小彩蛋； 提示：</div><div>homo（需在未点亮按钮状态触发）</div><div>【需要重新恢复按钮为未点亮状态请进入 已安装脚本->编辑->开发者->重置到出厂->确定】</div><div>2、修改/增加默认主题色。</div>"}, {"version": "V1.0.2", "content": "<div>1、修改并加宽界面，调整部分css，使Sweetalert2界面更美观，更与原版相近；</div><div>2、修改部分提示文字，使文字更容易复制。</div>"}, {"version": "V1.0.1", "content": "<div>1、去除更新提示；</div><div>2、更新Sweetalert2至11版本；</div><div>3、部分CDN节点更换为jsdelivr。</div>"}, {"version": "V1.0.0", "content": "<div>1、增加\"注入\"功能（bushi）；</div><div>2、去除广告。</div>"}];
				...
				didOpen: () => {{
					...
					if (select) {{
						select.addEventListener('change', (e) => {{
							...
							const v = versionData.find(item => item.version === version);
							if (v) {{
								document.getElementById('version-content').innerHTML = v.content;
								document.getElementById('version-title').textContent = version;
							}}
						}});
					}}
				}},
		...
		addPageListener() {
			...
			$doc.on("change", ".listener-default-mode", async function (e) {
				base.setValue("setting_default_mode", e.currentTarget.value);
				base.refreshDefaultModeButtons();
			});
			$doc.on("change", ".listener-pan-toggle", async function (e) {
				let list = base.getValue("setting_enabled_pan");
				list[e.currentTarget.dataset.pan] = e.currentTarget.checked;
				base.setValue("setting_enabled_pan", list);
			});
			$doc.on("click", ".listener-pan-toggle-all", async function (e) {
				let list = base.getValue("setting_enabled_pan");
				let allChecked = Object.values(list).every(v => v === true);
				let newValue = !allChecked;
				for (let key in list) { list[key] = newValue; }
				base.setValue("setting_enabled_pan", list);
				// 更新所有 checkbox 状态
				$(e.currentTarget).closest(".pl-setting-item").find(".listener-pan-toggle").each(function() {
					$(this).prop("checked", newValue);
				});
				$(e.currentTarget).text(newValue ? "全不选" : "全选");
			});
			$doc.on("click", ".listener-default-download", async function (e) {
				let defaultMode = base.getValue("setting_default_mode");
				if (!defaultMode) { message.warning("请先在助手设置中选择默认下载方式"); return; }
				temp.mode = defaultMode;
...
.pl-ios-switches{display:flex;flex-wrap:wrap;gap:6px 12px}
.pl-ios-switch-item{display:flex;align-items:center;gap:6px;cursor:pointer;font-size:13px;white-space:nowrap}
.pl-ios-switch-item input[type="radio"],.pl-ios-switch-item input[type="checkbox"]{appearance:none;-webkit-appearance:none;width:36px;height:20px;border-radius:10px;background:#ccc;position:relative;cursor:pointer;transition:background-color.3s;flex-shrink:0}
.pl-ios-switch-item input[type="radio"]:checked,.pl-ios-switch-item input[type="checkbox"]:checked{background:${temp.color}}
.pl-ios-switch-item input[type="radio"]::before,.pl-ios-switch-item input[type="checkbox"]::before{content:'';position:absolute;width:16px;height:16px;border-radius:50%;background:#fff;top:2px;left:2px;transition:transform.3s;box-shadow:0 1px 3px rgba(0,0,0,.2)}
.pl-ios-switch-item input[type="radio"]:checked::before,.pl-ios-switch-item input[type="checkbox"]:checked::before{transform:translateX(16px)}
.pl-ios-switch-label{user-select:none}
.pl-default-btn{display:inline-flex;align-items:center;gap:4px;cursor:pointer;white-space:nowrap}
...
`);
	...
	let $baidu = {
		...
		addButton() {
			base.waitForKeyElements(config.$baidu.mount.home, (element) => {
				...
				let $defaultBtn = $(`<div class="g-dropdown-button pl-default-btn listener-default-download" style="margin-left:5px;">
					<div class="g-button g-button-blue color-button"><span class="g-button-right"><em class="icon icon-download" style="color:#fff;"></em><span class="text pl-default-mode-text" style="width:60px;">${base.getDefaultModeName()}</span></span></div>
				</div>`);
				element.prepend($defaultBtn);
			})
			base.waitForKeyElements(config.$baidu.mount.main, (element) => {
				...
				element.prepend($button);
				let $defaultBtn2 = $(`<div class="wp-s-agile-tool-bar__h-group pl-default-btn listener-default-download" style="margin-left:5px;">
					<div class="wp-s-agile-tool-bar__h-action is-need-left-sep is-main color-button">
						<button type="button" class="u-button nd-file-list-toolbar-action-item u-button--primary u-button--small is-round is-has-icon pl-default-btn color-button">
							<i class="u-icon u-icon-download"></i>
							<span class="pl-default-mode-text">${base.getDefaultModeName()}</span>
						</button>
					</div>
				</div>`);
				element.prepend($defaultBtn2);
			})
			base.waitForKeyElements(config.$baidu.mount.main, (element) => {
				temp.page = temp.main.detectPage();
				...
				element.prepend($button);
				let $defaultBtn3 = $(`<div class="wp-s-agile-tool-bar__h-group pl-default-btn listener-default-download" style="margin-left:5px;">
					<div class="wp-s-agile-tool-bar__h-action is-need-left-sep is-main color-button">
						<button type="button" class="u-button nd-file-list-toolbar-action-item u-button--primary u-button--small is-round is-has-icon pl-default-btn color-button" style="font-size:14px;font-weight:700">
							<i class="u-icon u-icon-download"></i>
							<span class="pl-default-mode-text">${base.getDefaultModeName()}</span>
						</button>
					</div>
				</div>`);
				element.prepend($defaultBtn3);
			})
			base.waitForKeyElements(config.$baidu.mount.share, (element) => {
				...
				let $defaultBtn4 = $(`<a class="g-button tools-share-V20-btn save_btn pl-default-btn color-button listener-default-download" style="padding:0;margin-left:5px;">
					<span class="g-button-right" style="padding-left:10px">
						<em class="icon icon-download" style="color:#fff;line-height:27px"></em>
						<span class="text pl-default-mode-text" style="width:auto;">${base.getDefaultModeName()}</span>
					</span>
				</a>`);
				element.after($defaultBtn4);
			})
		},
		addInitButton() {
			base.waitForKeyElements(config.$baidu.mount.home, (element) => {
	...
	let $aliyun = {
		...
		addButton() {
			base.waitForKeyElements(config.$aliyun.mount.home, (element) => {
				...
				let $defaultBtn = $(`<div class="ali-button pl-default-btn listener-default-download" style="margin-left:5px;">
					<span data-role="icon" data-render-as="svg" class="icon">${temp.main.svg}<span class="pl-default-mode-text">${base.getDefaultModeName()}</span></span>
				</div>`);
				element.append($defaultBtn);
			})
			base.waitForKeyElements(config.$aliyun.mount.share, (element) => {
				...
				element.prepend($button);
				let $defaultBtn2 = $(`<div class="ali-button pl-default-btn listener-default-download" style="margin-left:5px;height:36px;width:auto;padding:1px 30px;">
					<span data-role="icon" data-render-as="svg" class="icon">${temp.main.svg}<span class="pl-default-mode-text">${base.getDefaultModeName()}</span></span>
				</div>`);
				element.prepend($defaultBtn2);
			})
	...
	let $mcloud = {
		...
		addButton() {
			base.waitForKeyElements(config.$mcloud.mount.home, (element) => {
				...
				element.prepend($button);
				let $defaultBtn = $(`<div class="pl-button mcloud-button btn-top pl-default-btn listener-default-download" style="margin-left:5px;">
					<span class="mcloud-btn pl-default-mode-text">${base.getDefaultModeName()}</span>
				</div>`);
				element.prepend($defaultBtn);
			})
			base.waitForKeyElements(config.$mcloud.mount.share, (element) => {
				...
				element.prepend($button);
				let $defaultBtn2 = $(`<div class="pl-button mcloud-share-button pl-default-btn listener-default-download" style="margin-left:5px;">
					<span class="mcloud-btn pl-default-mode-text">${base.getDefaultModeName()}</span>
				</div>`);
				element.prepend($defaultBtn2);
			})
	...
	let $tcloud = {
		...
		addButton() {
			...
			base.waitForKeyElements(config.$tcloud.mount.home, (element) => {
				...
				element.prepend($button);
				let $defaultBtn = $(`<div class="pl-button tcloud-button pl-default-btn listener-default-download" style="margin-left:5px;">
					<span class="pl-default-mode-text">${base.getDefaultModeName()}</span>&nbsp;
					<i aria-label="icon:caret-down" class="anticon anticon-caret-down">
						<svg viewBox="0 0 1024 1024" data-icon="caret-down" width="1em" height="1em" fill="currentColor" aria-hidden="true" focusable="false">
							<path d="M840.4 300H183.6c-19.7 0-30.7 20.8-18.5 35l328.4 380.8c9.4 10.9 27.5 10.9 37 0L858.9 335c12.2-14.2 1.2-35-18.5-35z"></path>
	...
	let $xunlei = {
		...
		addButton() {
			base.waitForKeyElements(config.$xunlei.mount.home, (element) => {
				...
				element.prepend($button);
				let $defaultBtn = $(`<div class="xunlei-button pl-default-btn listener-default-download" style="margin-left:5px;"><i class="xlpfont xlp-download"></i><span class="pl-default-mode-text" style="font-size:13px;margin-left:6px;">${base.getDefaultModeName()}</span></div>`);
				element.prepend($defaultBtn);
			})
			base.waitForKeyElements(config.$xunlei.mount.share, (element) => {
				...
				element.prepend($button);
				let $defaultBtn2 = $(`<div class="xunlei-button pl-default-btn listener-default-download" style="margin-left:5px;"><i class="xlpfont xlp-download"></i><span class="pl-default-mode-text" style="font-size:13px;margin-left:6px;">${base.getDefaultModeName()}</span></div>`);
				element.prepend($defaultBtn2);
			})
		},
		addInitButton() {
	...
	let $quark = {
		...
		addButton() {
			base.waitForKeyElements(config.$quark.mount.home, (element) => {
				...
				element.prepend($button);
				let $defaultBtn = $(`<button type="button" class="ant-btn ant-btn-primary quark-button pl-default-btn listener-default-download" style="margin-right:16px;">
					<img class="btn-icon" src="${temp.main.svg}">
					<span class="pl-default-mode-text">${base.getDefaultModeName()}</span>
				</button>`);
				element.prepend($defaultBtn);
			...
			base.waitForKeyElements(config.$quark.mount.share, (element) => {
				...
				element.append($button);
				let $defaultBtn2 = $(`<button type="button" class="ant-btn btn-file ant-btn-primary pl-default-btn quark-button listener-default-download" style="height:36px;margin-left:16px;border-radius:6px;display:inline-block;">
					<img class="btn-icon" src="${temp.main.svg}"><span class="pl-default-mode-text">${base.getDefaultModeName()}</span>
				</button>`);
				element.append($defaultBtn2);
			})
		},
		addInitButton() {
			base.waitForKeyElements(config.$quark.mount.home, (element) => {
	...
	let $uc = {
		...
		addButton() {
			base.waitForKeyElements(config.$uc.mount.home, (element) => {
				...
				element.prepend($button);
				let $defaultBtn = $(`<div class="ant-dropdown-trigger pl-default-btn listener-default-download" style="margin-right:10px;display:inline-block;">
					<button type="button" class="uc-button ant-btn btn-file ant-btn-primary">
						<img class="uc-btn-icon" src="${temp.main.svg}"><span class="pl-default-mode-text">${base.getDefaultModeName()}</span>
					</button>
				</div>`);
				element.prepend($defaultBtn);
			})
			base.waitForKeyElements(config.$uc.mount.share, (element) => {
				...
				element.append($button);
				let $defaultBtn2 = $(`<div class="ant-dropdown-trigger pl-default-btn listener-default-download" style="margin-left:10px;display:inline-block;">
					<button type="button" class="uc-button ant-btn btn-file ant-btn-primary" style="height:40px;">
						<img class="uc-btn-icon" src="${temp.main.svg}"><span class="pl-default-mode-text">${base.getDefaultModeName()}</span>
					</button>
				</div>`);
	...
	let $123pan = {
		...
		addButton() {
			base.waitForKeyElements(config.$123pan.mount.home, (element) => {
				...
				element.prepend($button);
				let $defaultBtn = $(`<button type="button" class="ant-btn ${[...document.querySelector(`[class*="ant-btn css-"]`).classList].find(c => /^css-[a-z0-9]+$/.test(c))} ant-btn-primary ant-btn-color-primary ant-btn-variant-solid mfy-button upload-button pl-default-btn listener-default-download color-button" style="user-select: text !important;margin-left:5px;">
					<svg class="icon home-operator-icon-upload" aria-hidden="true"><use xlink:href="#general_download_16_1"></use></svg>
					<span class="pl-default-mode-text">${base.getDefaultModeName()}</span>
				</button>`);
				element.prepend($defaultBtn);
			...
			base.waitForKeyElements(config.$123pan.mount.share, (element) => {
				...
				element.append($button);
				let $defaultBtn2 = $(`<div class="register pl-default-btn listener-default-download color-button" style="width:100px;margin-left:5px;">
					<svg class="icon" aria-hidden="true" style="color:rgb(255, 255, 255);margin-right:5px;"><use xlink:href="#top_btn_download2"></use></svg><span class="pl-default-mode-text">${base.getDefaultModeName()}</span>
				</div>`);
				element.append($defaultBtn2);
			})
			base.waitForKeyElements(config.$123pan.mount.shareNew, (element) => {
				...
				element.append($button);
				let $defaultBtn3 = $(`<button type="button" class="ant-btn ${[...document.querySelector(`[class*="ant-btn css-"]`).classList].find(c => /^css-[a-z0-9]+$/.test(c))} ant-btn-primary ant-btn-color-primary ant-btn-variant-solid mfy-button pl-default-btn listener-default-download color-button" style="user-select: text !important;margin-left:5px;">
					<svg class="icon" aria-hidden="true" style="color: rgb(255, 255, 255);"><use xlink:href="#general_download_16_1"></use></svg>
					<span class="pl-default-mode-text">${base.getDefaultModeName()}</span>
				</button>`);
				element.append($defaultBtn3);
			})
		},
		addInitButton() {
			base.waitForKeyElements(config.$123pan.mount.home, (element) => {
	...
	let main = {
		async init() {
			...
			// 检查当前网盘是否启用
			let currentPan = null;
			if (/(pan|yun).baidu.com/.test(location.host)) currentPan = "$baidu";
			else if (/www.(aliyundrive|alipan).com/.test(location.host)) currentPan = "$aliyun";
			else if (/(yun|caiyun).139.com/.test(location.host)) currentPan = "$mcloud";
			else if (/cloud.189.cn/.test(location.host)) currentPan = "$tcloud";
			else if (/pan.xunlei.com/.test(location.host)) currentPan = "$xunlei";
			else if (/pan.quark.cn/.test(location.host)) currentPan = "$quark";
			else if (/drive.uc.cn/.test(location.host)) currentPan = "$uc";
			else if (/(www|login).(123(pan|684|865|952|912).com|123pan.cn)/.test(location.host)) currentPan = "$123pan";
			if (currentPan && base.getValue("setting_enabled_pan")[currentPan] === false) return;
